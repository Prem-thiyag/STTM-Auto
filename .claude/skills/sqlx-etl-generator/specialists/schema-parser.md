...
1. Read the markdown file. Identify the database name it declares (look for an
   explicit heading or statement like "Database: X"; if genuinely absent, use the
   input's filename stem as a last resort and note the fallback in the table
   `description` field of the first table — do not silently invent a name that
   isn't traceable to the document).
